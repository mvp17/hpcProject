...
int convolve2D(ImagenData, ImagenData, int, int, float*, int, int);
...
///////////////////////////////////////////////////////////////////////////////
int convolve2D(ImagenData in, ImagenData out, int dataSizeX, int dataSizeY,
                float* kernel, int kernelSizeX, int kernelSizeY)
{
    int i, j, m, n;
    int *R_inPtr, *R_inPtr2, *R_outPtr;
    int *G_inPtr, *G_inPtr2, *G_outPtr;
    int *B_inPtr, *B_inPtr2, *B_outPtr;
    float *kPtr;
    int kCenterX, kCenterY;
    int rowMin, rowMax;                             // to check boundary of input array
    int colMin, colMax;                             //
    float Rsum, Gsum, Bsum;                                      // temp accumulation buffer
    ...
    R_inPtr = R_inPtr2 = &in->R[dataSizeX * kCenterY + kCenterX];  // note that  it is shifted (kCenterX, kCenterY),
    R_outPtr = out->R;
    G_inPtr = G_inPtr2 = &in->G[dataSizeX * kCenterY + kCenterX];  // note that  it is shifted (kCenterX, kCenterY),
    G_outPtr = out->G;
    B_inPtr = B_inPtr2 = &in->B[dataSizeX * kCenterY + kCenterX];  // note that  it is shifted (kCenterX, kCenterY),
    B_outPtr = out->B;
	...
	#pragma omp parallel shared()private(i,j,m,n,inPtr, inPtr2, outPtr, kPtr, rowMin, rowMax, colMin, colMax)
    {
    ...
    for(i= 0; i < dataSizeY; ++i)                   // number of rows
    {
        ...
        for(j = 0; j < dataSizeX; ++j)              // number of columns
        {
            ...
            Rsum = 0; Gsum=0; Bsum=0;                           // set to 0 before accumulate
            ...
            for(m = 0; m < kernelSizeY; ++m)        // kernel rows
            {
                ...
                R_inPtr -= dataSizeX;                 // move input data 1 raw up
                G_inPtr -= dataSizeX;
                B_inPtr -= dataSizeX;
            }

            // convert integer number
            if(Rsum >= 0) *R_outPtr = (int)(Rsum + 0.5f);
            else *R_outPtr = (int)(Rsum - 0.5f);

            if(Gsum >= 0) *G_outPtr = (int)(Gsum + 0.5f);
            else *G_outPtr = (int)(Gsum - 0.5f);
            
            if(Bsum >= 0) *B_outPtr = (int)(Bsum + 0.5f);
            else *B_outPtr = (int)(Bsum - 0.5f);
            

            
            kPtr = kernel;                          // reset kernel to (0,0)
            R_inPtr = ++R_inPtr2;                       // next input
            G_inPtr = ++G_inPtr2;                       // next input
            B_inPtr = ++B_inPtr2;                       // next input


            ++R_outPtr;                               // next output
            ++G_outPtr;                               // next output
            ++B_outPtr;                               // next output
        }
    }
}
    return 0;
...
int main(int argc, char **argv)
{
    ...
    convolve2D(source, output, source->width, source->height, kern->vkern, kern->kernelX, kern->kernelY);
